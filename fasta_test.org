...
#+end_src
#+begin_export latex
\subsubsection{Method \texttt{KeepOnlyATGC}}
...
\subsubsection{\texttt{ReadAndConcatenate}}
To test \texttt{ReadAndConcatenate}, we read the test files
(Table~\ref{tab:testfiles}).
#+end_export
#+begin_src go <<Testing functions>>=
...
#+end_src
#+begin_export latex
We define arrays of expected values: number of bytes in the data,
including sentinels, and anticipated contents of the headers.
#+end_export
